	      ...
	      struct point *p, int count)
{
  ...
  for (i = 0; i < count; i++)
    {
      quad1[i][0] = (GLdouble) (p[i].x + gr->p.x);
      quad1[i][1] = (GLdouble) (p[i].y + gr->p.y);
      quad1[i][2] = 0;
    }
	  ...
	  if (color)
	    {
	      ...
	      glDrawPixels (g->w+2, g->h+2, GL_BGRA, GL_UNSIGNED_BYTE, shadow);
	      g_free (shadow);
	    }
	}
      x += g->dx;
      ...
      if (g->w && g->h)
	{
	  if (color)
	      ...
	      if (bg)
		{
		  ...
		  gr->freetype_methods.get_glyph (g, glyph, 24, stride * 4,
						  &black, &white,
						  &white);
		  g_free (glyph);
		}

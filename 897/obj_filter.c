...
int parse_obj_filter(const char*input, struct obj_filter_t* out)
{
  ...
  *expr = 0;
  if (*curr_ch==']') {
    ...
    //skip whitespaces
    while(isspace(*curr_ch)) {++curr_ch;}
    char*end1 = curr_ch; //possible end of expression if no idx specified
...
struct attr* filter_object(struct attr* root,char*iter_type,char*expr, int idx)
{
  ...
  struct object_func *obj_func_root = object_func_lookup(((struct attr*)root)->type);
  struct object_func *obj_func_iter = object_func_lookup(attr_from_name(iter_type));
  ...
  struct attr_iter * iter = obj_func_root->iter_new(NULL);
...
static int parse_bool_expr(struct ctx*the_ctx)
{
  ...
  //printf("syntax error : parse_bool_expr\n");
  the_ctx->error = ERR_SYNTAX;
  return 0;
}
